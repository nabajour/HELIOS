...
def conv_check(quant):
    ...
    T_ad_lim = quant.T_lay[quant.nlayer] * \
        (quant.p_lay[0] / quant.p_int[0]) ** (quant.kappa_int[0] * (1 + 1e-6))
        ...
        for n in range(len(start_layers)):
            ...
            if quant.T_star >= 10:
                ...
                if quant.input_dampara == "auto":
                    # 16 is found to lead to the fastest convergence hence taking as nominal value
                    quant.dampara = 16
                else:
            ...
            # same procedure without a stellar energy source
                ...
                if quant.input_dampara == "auto":
                    # 1024 is found to lead to the fastest convergence hence taking as nominal value
                    quant.dampara = 1024
                else:
                    quant.dampara = int(quant.input_dampara)
...
def mark_convective_layers(quant, stitching):
    ...
    for i in range(quant.nlayer - 1):
        ...
        # temperature inversion directly next to adiabat is unphysical. this fix helps with potential kinks.
...
def construct_grid(quant):
    press_levels = [quant.p_boa * (quant.p_toa/quant.p_boa) **
                    (i/(2 * quant.nlayer - 1)) for i in range(2 * quant.nlayer)]

    quant.p_lay = [press_levels[i] for i in range(1, 2 * quant.nlayer, 2)]

    quant.p_int = [press_levels[i] for i in range(0, 2 * quant.nlayer, 2)]

    quant.p_int.append(quant.p_toa * (quant.p_toa/quant.p_boa)
                       ** (1/(2 * quant.nlayer - 1)))
    for i in range(quant.nlayer):
